...
import { QuickAddLead } from '@/components/QuickAdd'
...
export default function HomePage() {
  ...
  const [healthData, setHealthData] = useState<Record<string, {score: 'green'|'yellow'|'red', reason: string}>>({})
  ...
  useEffect(() => {
    ...
    // Realtime: reload when new lead comes in
    const channel = supabase.channel('homepage-leads')
  ...
  async function load() {
    ...
    const list = ws || []
    setClients(list)
    const clientList = list.filter((c: any) => !c.is_personal)
    const mrr = clientList.reduce((s: number, c: any) => s + (Number(c.retainer) || 0), 0)
    const active = clientList.filter((c: any) => c.status === 'Aktiv' || !c.status).length
    ...
    const clientIds = list.filter((c: any) => !c.is_personal).map((c: any) => c.id)
    if (clientIds.length > 0) {
      ...
      const [{ data: overdueTasks }, { data: recentActivity }] = await Promise.all([
        supabase.from('tasks').select('workspace_id').neq('status', 'Erledigt').lt('faellig', todayStr).in('workspace_id', clientIds),
        supabase.from('leads').select('workspace_id, created_at').gte('created_at', day30).in('workspace_id', clientIds).order('created_at', { ascending: false }),
      ])

      const overdueCount: Record<string, number> = {}
      ;(overdueTasks || []).forEach((t: any) => { overdueCount[t.workspace_id] = (overdueCount[t.workspace_id] || 0) + 1 })

      const lastActivity: Record<string, string> = {}
      ;(recentActivity || []).forEach((l: any) => { if (!lastActivity[l.workspace_id]) lastActivity[l.workspace_id] = l.created_at })

      const scores: Record<string, {score: 'green'|'yellow'|'red', reason: string}> = {}
      clientIds.forEach((id: string) => {
        const overdue = overdueCount[id] || 0
        const lastLead = lastActivity[id]
        const hasRecent14 = lastLead && lastLead >= day14
        const hasRecent30 = lastLead && lastLead >= day30

        if (overdue >= 3) {
          scores[id] = { score: 'red', reason: `${overdue} überfällige Aufgaben` }
        } else if (overdue > 0 && !hasRecent30) {
          scores[id] = { score: 'red', reason: 'Keine Aktivität + überfällige Aufgaben' }
        } else if (overdue > 0 || !hasRecent14) {
          scores[id] = { score: 'yellow', reason: overdue > 0 ? `${overdue} überfällige Aufgabe${overdue > 1 ? 'n' : ''}` : 'Kein neuer Lead seit 14+ Tagen' }
        } else {
          scores[id] = { score: 'green', reason: 'Alles im grünen Bereich' }
        }
      })
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f7f7f5' }}>
      ...
      <div style={{ padding: '32px 28px 48px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        {/* Stats */}
        <div className="animate-card stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14, marginBottom: 32, animationDelay: '0.05s' }}>
          {loading ? <SkeletonStats /> : <>
            {[
              { label: 'Aktive Clients', value: stats.active, sub: `${clients.filter((c:any)=>!c.is_personal).length} Gesamt`, icon: '◎', color: '#6366f1' },
              ...
              { label: 'Client Health', value: (() => { const vals = Object.values(healthData); const red = vals.filter(v => v.score === 'red').length; const yellow = vals.filter(v => v.score === 'yellow').length; if (red > 0) return `${red} Problem${red > 1 ? 'e' : ''}`; if (yellow > 0) return `${yellow} Check`; return vals.length > 0 ? 'Alles gut' : '—' })(), sub: (() => { const vals = Object.values(healthData); const green = vals.filter(v => v.score === 'green').length; return `${green} von ${vals.length} im Grünen` })(), icon: '◉', color: (() => { const vals = Object.values(healthData); if (vals.some(v => v.score === 'red')) return '#ef4444'; if (vals.some(v => v.score === 'yellow')) return '#f59e0b'; return '#22c55e' })() },
            ].map((s, i) => (
        ...
        {loading ? (
          ...
          <>
            {/* Mein Workspace — persönlicher Bereich */}
            {filtered.filter(c => c.is_personal).map(c => (
              <Link key={c.id} href={`/client/${c.id}`} style={{ display: 'block', marginBottom: 28 }}>
                <div className="animate-card" style={{ background: '#1a1a1a', borderRadius: 14, overflow: 'hidden', cursor: 'pointer', transition: 'box-shadow 0.15s, transform 0.12s', animationDelay: '0s' }}
                  onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.boxShadow = '0 8px 28px rgba(0,0,0,0.2)'; el.style.transform = 'translateY(-2px)' }}
                  onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.boxShadow = 'none'; el.style.transform = 'none' }}
                >
                  <div style={{ padding: '20px 22px', display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ width: 42, height: 42, borderRadius: 10, background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18 }}>
                      ⚡
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', letterSpacing: '-0.2px' }}>Mein Workspace</div>
                      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>Outreach · Pipeline · Leads</div>
                    </div>
            ...
            {/* Clients */}
            {filtered.filter(c => !c.is_personal).length > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <h2 style={{ fontSize: 15, fontWeight: 700, color: '#1a1a1a', letterSpacing: '-0.3px' }}>Deine Clients</h2>
                <span style={{ fontSize: 12, color: '#bbb' }}>{filtered.filter(c => !c.is_personal).length} {filtered.filter(c => !c.is_personal).length === 1 ? 'Client' : 'Clients'}</span>
              </div>
            )}

            {filtered.filter(c => !c.is_personal).length === 0 && !search ? (
              ...
              <div className="client-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
                {filtered.filter(c => !c.is_personal).map((c, i) => {
                        ...
                        <div style={{ padding: '16px 18px 18px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                            ...
                            {(() => {
                              ...
                              const col = h?.score === 'green' ? '#22c55e' : h?.score === 'yellow' ? '#f59e0b' : h?.score === 'red' ? '#ef4444' : '#d1d5db'
                              const bg = h?.score === 'green' ? '#f0fdf4' : h?.score === 'yellow' ? '#fffbeb' : h?.score === 'red' ? '#fef2f2' : '#f9fafb'
                              const label = h?.score === 'green' ? 'Gut' : h?.score === 'yellow' ? 'Check' : h?.score === 'red' ? 'Problem' : '—'
                              return (
                                <div title={h?.reason || 'Keine Daten'} style={{ display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0, background: bg, border: `1px solid ${col}30`, borderRadius: 99, padding: '3px 8px 3px 6px', cursor: 'default' }}>
                                  <div style={{ width: 7, height: 7, borderRadius: 99, background: col, boxShadow: h?.score !== undefined ? `0 0 0 2px ${col}30` : 'none' }} />
                                  <span style={{ fontSize: 10.5, fontWeight: 600, color: col }}>{label}</span>
                                </div>
                              )
                            })()}
                          </div>
                ...
                <Link href="/client/new">
                  <div className="animate-card" style={{ background: 'transparent', borderRadius: 14, border: '1.5px dashed #ddd', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 148, gap: 8, transition: 'all 0.15s', animationDelay: `${filtered.filter(c => !c.is_personal).length * 0.06}s` }}
