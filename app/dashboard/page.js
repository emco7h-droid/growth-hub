      ...
      <div style={{ padding: '20px 24px' }}>
        ...
        {tab === 'overview' && <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 20 }}>
            <MetricCard label="Aktive Clients" value={activeClients} sub={workspaces.length + ' gesamt'} icon="📋" color={wsColor} trend={undefined} sparkData={undefined} onClick={() => router.push('/workspaces')} />
            <MetricCard label="Monatlicher MRR" value={`€${totalRetainer.toLocaleString('de-DE')}`} sub="Alle Clients" icon="💰" color="#059669" trend={undefined} sparkData={umsatzVerlauf.length >= 2 ? umsatzVerlauf : undefined} />
            <MetricCard label="Offene Leads" value={leads.filter(l => l.status !== 'Gewonnen' && l.status !== 'Verloren').length} sub={`${leads.length} gesamt · ${conv}% Conv.`} icon="🎯" color="#6366f1" trend={undefined} sparkData={leadVerlauf.length >= 2 ? leadVerlauf : undefined} onClick={() => router.push('/pipeline')} />
            <MetricCard label="Offene Aufgaben" value={openTasks} sub={highPrio > 0 ? `⚠️ ${highPrio} hoch priorisiert` : 'Keine kritischen'} icon="✅" color={highPrio > 0 ? '#dc2626' : '#059669'} trend={undefined} onClick={() => router.push('/tasks')} />
          </div>
        ...
        {tab === 'finance' && (
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14, marginBottom: 20 }}>
              <MetricCard label="Gesamt MRR" value={`€${totalRetainer.toLocaleString('de-DE')}`} icon="💰" color="#059669" />
              <MetricCard label="Bezahlte Rechnungen" value={`€${bezahlt.toLocaleString('de-DE')}`} icon="✅" color="#059669" />
              <MetricCard label="Ausstehend" value={`€${ausstehend.toLocaleString('de-DE')}`} icon="⏳" color={ausstehend > 0 ? '#f59e0b' : '#059669'} />
            </div>
            <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e1e4e8', overflow: 'hidden' }}>
