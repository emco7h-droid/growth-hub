...
// Simple workspace cache to avoid re-fetching on navigation
const wsCache: Record<string, any> = {}
...
export default function ClientLayout({ children, params }: { children: React.ReactNode; params: { id: string } }) {
  ...
  useEffect(() => {
    if (wsCache[params.id]) {
      setClient(wsCache[params.id])
    } else {
      supabase.from('workspaces').select('*').eq('id', params.id).single().then(({ data }) => {
        if (data) { wsCache[params.id] = data; setClient(data) }
      })
    }
