...
export default function PipelinePage({ params }: { params: { id: string } }) {
  ...
  if (loading) return (
    <div style={{ padding: '28px 32px' }}>
      <div style={{ height: 28, width: 200, background: '#ebebeb', borderRadius: 8, marginBottom: 20, animation: 'pulse 1.5s ease-in-out infinite' }} />
      <div style={{ display: 'flex', gap: 12 }}>
        {[1,2,3].map(i => (
          <div key={i} style={{ minWidth: 230, background: '#f5f5f5', borderRadius: 12, padding: 14, animation: 'pulse 1.5s ease-in-out infinite', animationDelay: i * 0.1 + 's' }}>
            <div style={{ height: 12, width: '60%', background: '#ebebeb', borderRadius: 6, marginBottom: 12 }} />
            {[1,2].map(j => <div key={j} style={{ height: 60, background: '#fff', borderRadius: 8, marginBottom: 8 }} />)}
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ padding: '28px 32px' }}>
      ...
      {tab === 'kanban' && (
        loading ? (
          <div style={{ display: 'flex', gap: 12 }}>
            {[1,2,3].map(i => (
              <div key={i} style={{ minWidth: 230, background: '#f9f9f9', borderRadius: 12, border: '1px solid #eee', padding: 12 }}>
                <div style={{ height: 14, background: '#ebebeb', borderRadius: 6, marginBottom: 10, width: '60%', animation: 'pulse 1.5s infinite' }} />
                {[1,2].map(j => <div key={j} style={{ height: 60, background: '#fff', borderRadius: 8, border: '1px solid #eee', marginBottom: 7 }} />)}
              </div>
            ))}
          </div>
        ) : stages.length === 0 ? (
