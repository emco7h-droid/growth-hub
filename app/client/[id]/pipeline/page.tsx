...
export default function PipelinePage({ params }: { params: { id: string } }) {
  const { client, stages, leads, loading, refresh } = useClientData()
  ...
  if (loading) return (
    <div style={{ padding: '28px 32px' }}>
      ...
      <div style={{ display: 'flex', gap: 14 }}>
        {[1,2,3,4].map(i => (
          <div key={i} className="skeleton-pulse" style={{ minWidth: 220, background: '#f3f4f6', borderRadius: 12, padding: 14 }}>
            <div style={{ height: 12, width: '55%', background: '#e5e7eb', borderRadius: 6, marginBottom: 14 }} />
            {[1,2,3].map(j => <div key={j} style={{ height: 58, background: '#fff', borderRadius: 8, marginBottom: 8, border: '1px solid #ebebeb' }} />)}
          </div>
        ))}
      </div>
    </div>
      ...
      {tab === 'kanban' && (
        stages.length === 0 ? (
          ...
          <div style={{ display: 'flex', gap: 12, overflowX: 'auto', paddingBottom: 8 }}>
            ...
            <div onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (dragging) { supabase.from('leads').update({ stage_id: null, status: 'Neu' }).eq('id', dragging).then(() => { setDragging(null); refresh('leads') }) } }} style={{ minWidth: 230, background: '#f9f9f9', borderRadius: 12, border: '1px solid #ebebeb' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer' }}>
                  <input type="checkbox" checked={stageForm.ist_gewonnen} onChange={e => setStageForm(p => ({ ...p, ist_gewonnen: e.target.checked, ist_verloren: false }))} />🏆 Gewonnen
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer' }}>
                  <input type="checkbox" checked={stageForm.ist_verloren} onChange={e => setStageForm(p => ({ ...p, ist_verloren: e.target.checked, ist_gewonnen: false }))} />✗ Verloren
                </label>
              </div>
