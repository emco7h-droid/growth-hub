  ...
  return (
    <div className="main-content" style={{ padding: '28px 28px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <button className="btn-primary" onClick={() => { setEditing(null); setForm({ name: '', email: '', telefon: '', unternehmen: '', quelle: 'OnePage.io', stage_id: '', deal_wert: '', notizen: '' }); setShowModal(true) }}>
          + Lead
        </button>
      </div>
              ...
              {filtered.map(lead => {
                ...
                return (
                  <tr key={lead.id} style={{ borderBottom: '1px solid #f9f9f9', transition: 'background 0.1s' }}
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', gap: 4 }}>
                        <button onClick={() => { setEditing(lead); setForm({ name: lead.name, email: lead.email || '', telefon: lead.telefon || '', unternehmen: lead.unternehmen || '', quelle: lead.quelle, stage_id: lead.stage_id || '', deal_wert: String(lead.deal_wert || ''), notizen: lead.notizen || '' }); setShowModal(true) }} className="btn-secondary" style={{ padding: '4px 10px', fontSize: 11.5 }}>Edit</button>
                        <button onClick={() => deleteLead(lead.id)} className="btn-danger" style={{ padding: '4px 8px', fontSize: 11.5 }}>×</button>
                      </div>
                    </td>
                  </tr>
                )
