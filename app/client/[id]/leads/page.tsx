              ...
              {filtered.map(lead => {
                ...
                return (
                  <tr key={lead.id} style={{ borderBottom: '1px solid #f9f9f9', transition: 'background 0.1s' }}
                    ...
                    <td style={{ padding: '11px 14px' }}>
                      <div style={{ display: 'flex', gap: 4 }}>
                        {allowManual && <button onClick={() => { setEditing(lead); setForm({ name: lead.name, email: lead.email || '', telefon: lead.telefon || '', instagram: (lead as any).instagram || '', youtube: (lead as any).youtube || '', nische: (lead as any).nische || '', aktuelles_produkt: (lead as any).aktuelles_produkt || '', loom_link: (lead as any).loom_link || '', quelle: lead.quelle || 'Instagram', stage_id: lead.stage_id || '', deal_wert: String(lead.deal_wert || ''), notizen: lead.notizen || '' }); setShowModal(true) }} className="btn-secondary" style={{ padding: '4px 10px', fontSize: 11.5 }}>Edit</button>}
