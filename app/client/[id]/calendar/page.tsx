...
export default function CalendarPage({ params }: { params: { id: string } }) {
  ...
  async function load() {
    const [cr, wr] = await Promise.all([
      supabase.from('content_calendar').select('*').eq('workspace_id', params.id).order('geplant_fuer', { ascending: true }),
      supabase.from('workspaces').select('*').eq('id', params.id).single(),
    ])
    setItems(cr.data || [])
    setClient(wr.data)
    setGoogleUrl(wr.data?.google_calendar_url || '')
    setLoading(false)
  }
  ...
  if (loading) return (
    <div style={{ padding: '28px 32px' }}>
      ...
      {[1,2,3,4].map(i => (
        <div key={i} style={{ height: 56, background: '#f5f5f5', borderRadius: 10, marginBottom: 10, animation: 'pulse 1.5s ease-in-out infinite', animationDelay: `${i * 0.08}s` }} />
      ))}
    </div>
  )
