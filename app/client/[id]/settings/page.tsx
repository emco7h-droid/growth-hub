      ...
      <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #ebebeb', padding: '20px 24px', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: 8, padding: '14px 16px' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#6d28d9', marginBottom: 6 }}>🔐 Client Portal</div>
            <div style={{ fontSize: 12, color: '#7c3aed', marginBottom: 12 }}>
              Gib dem Client Zugang zu seinem eigenen Portal. Er sieht dort seine Leads und Projektziele.
            </div>
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: '#555', display: 'block', marginBottom: 5 }}>Email des Clients (für den Login)</label>
              <input
                value={(form as any).portal_email || ''}
                onChange={e => setForm((p: any) => ({ ...p, portal_email: e.target.value }))}
                placeholder="client@example.com"
                type="email"
                style={{ width: '100%', padding: '8px 12px', border: '1px solid #ddd6fe', borderRadius: 8, fontSize: 13, boxSizing: 'border-box', marginBottom: 8 }}
              />
              <div style={{ fontSize: 11, color: '#8b5cf6' }}>
                Nach dem Speichern: Geh in Supabase → Authentication → Users → "Invite user" mit dieser Email. Der Client bekommt dann eine Email und kann sich einloggen.
              </div>
              {(form as any).portal_email && (
                <div style={{ marginTop: 10, padding: '8px 12px', background: '#ede9fe', borderRadius: 7, fontSize: 11, color: '#6d28d9' }}>
                  Portal URL: <strong>{typeof window !== 'undefined' ? window.location.origin : ''}/portal/{params.id}</strong>
                </div>
              )}
            </div>
          </div>
          ...
          {/* Manual leads toggle */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: '#f9f9f9', borderRadius: 10, border: '1px solid #eaeaea' }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a', marginBottom: 2 }}>Manuelles Hinzufügen von Leads</div>
              <div style={{ fontSize: 11.5, color: '#aaa' }}>
                {form.allow_manual_leads ? 'Leads können manuell eingetragen werden (z.B. für eigene Clients)' : 'Nur über Webhook / OnePage.io / Calendly'}
              </div>
            </div>
            <button onClick={() => setForm(p => ({ ...p, allow_manual_leads: !p.allow_manual_leads }))} style={{
              width: 44, height: 24, borderRadius: 99, border: 'none', cursor: 'pointer', flexShrink: 0,
              background: form.allow_manual_leads ? '#22c55e' : '#d1d5db', transition: 'background 0.2s', position: 'relative',
            }}>
              <div style={{
                width: 18, height: 18, borderRadius: 99, background: '#fff',
                position: 'absolute', top: 3, left: form.allow_manual_leads ? 23 : 3,
