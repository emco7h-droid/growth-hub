...
export default function SettingsPage({ params }: { params: { id: string } }) {
  ...
  const [form, setForm] = useState({ name: '', nische: '', retainer: '', status: 'Aktiv', start_date: '', modell: '1 zu 1', color: '#6366f1', client_email: '', calendly_url: '', website: '', notizen: '', allow_manual_leads: true })
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)

  useEffect(() => {
    supabase.from('workspaces').select('*').eq('id', params.id).single().then(({ data }) => {
      if (data) setForm({ name: data.name || '', nische: data.nische || '', retainer: String(data.retainer || ''), status: data.status || 'Aktiv', start_date: data.start_date || '', modell: data.modell || '1 zu 1', color: data.color || '#6366f1', client_email: data.client_email || '', calendly_url: data.calendly_url || '', website: data.website || '', notizen: data.notizen || '', allow_manual_leads: data.allow_manual_leads !== false })
    })
  }, [params.id])
